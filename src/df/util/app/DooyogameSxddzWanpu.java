        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case MSG_SPEND_SUCCESS:
                    if (null != mContext) {
                        Toast.makeText(mContext, "消费成功", Toast.LENGTH_SHORT).show();
                    }
                    break;
                case MSG_SPEND_FAIL:
                    if (null != mContext) {
//                                Toast.makeText(mContext, "消费失败", Toast.LENGTH_SHORT).show();
                    }
                    break;
...
//endif
    ...
    public static void initAdSdk(Context context) {
        if (!mAdInitFlag) {
            ...
            //todo: wulong万普总是有问题，全在一个Activity里初始化
...
//        id = ResourceUtil.getIdResourceIdFromName(context, "sxddz_ad_banner");
//        LinearLayout container = (LinearLayout) (((Activity) context).findViewById(id));
//        new MiniAdView(context, container).DisplayAd(7); //默认10秒切换一次广告
